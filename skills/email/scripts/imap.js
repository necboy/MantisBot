...
// Create IMAP connection config from account config
...
async function checkEmails(account, mailbox = 'INBOX', limit = 10, recentTime = null, unreadOnly = false) {
...
async function fetchEmail(account, uid, mailbox = 'INBOX') {
  const imap = await connect(account);
...
async function downloadAttachments(account, uid, mailbox = 'INBOX', outputDir = '.', specificFilename = null) {
  const imap = await connect(account);
...
async function markAsRead(account, uids, mailbox = 'INBOX') {
  const imap = await connect(account);
...
async function markAsUnread(account, uids, mailbox = 'INBOX') {
  const imap = await connect(account);
  ...
  try {
    ...
    switch (command) {
      ...
      case 'fetch':
        ...
        result = await fetchEmail(account, positional[0], options.mailbox);
        break;
      ...
      case 'download':
        ...
        result = await downloadAttachments(account, positional[0], options.mailbox, options.dir || '.', options.file || null);
        break;

      case 'search':
        result = await searchEmails(account, options);
        break;
      ...
      case 'mark-read':
        ...
        result = await markAsRead(account, positional, options.mailbox);
        break;
      ...
      case 'mark-unread':
        ...
        result = await markAsUnread(account, positional, options.mailbox);
        break;

      case 'list-mailboxes':
        result = await listMailboxes(account);
        break;
      ...
      default:
        ...
        console.error('');
        console.error('Usage:');
        console.error('  check [--account <id>] [--limit 10] [--mailbox INBOX] [--recent 2h] [--unseen]');
        console.error('  fetch <uid> [--account <id>] [--mailbox INBOX]');
        console.error('  download <uid> [--account <id>] [--mailbox INBOX] [--dir <path>] [--file <filename>]');
        console.error('  search [--account <id>] [--unseen] [--from <email>] [--subject <text>] [--recent 2h] [--limit 20]');
        console.error('  mark-read <uid> [uid2...] [--account <id>] [--mailbox INBOX]');
        console.error('  mark-unread <uid> [uid2...] [--account <id>] [--mailbox INBOX]');
        console.error('  list-mailboxes [--account <id>]');
        process.exit(1);
    }
