import { X, File, Folder, Mail } from 'lucide-react';
import type { ContextReference } from '../types/context-reference';

interface ContextReferenceTagsProps {
  references: ContextReference[];
  onRemove: (id: string) => void;
  onClear: () => void;
}

function ReferenceIcon({ item }: { item: ContextReference }) {
  if (item.source === 'email') return <Mail className="w-3 h-3" />;
  if (item.type === 'directory') return <Folder className="w-3 h-3" />;
  return <File className="w-3 h-3" />;
}

export function ContextReferenceTags({ references, onRemove, onClear }: ContextReferenceTagsProps) {
